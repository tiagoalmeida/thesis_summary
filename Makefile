...
.tex.pdf:
	...
	while grep 'Rerun to get cross-references right.' $*.log ; do \
	  $(PDFLATEX) $< $@ || { del -f $*.aux $*.idx && false ; } ; done

.tex.dvi:
	del -f $*.ps $*.pdf
	$(LATEX) $< || { del -f $*.dvi $*.aux $*.idx && false ; }
	if grep 'There were undefined references' $*.log ; then \
	  $(BIBTEX) $(TARGET); $(LATEX) $< $@; fi
	while grep 'Rerun to get cross-references right.' $*.log ; do \
	  $(LATEX) $< || { del -f $*.dvi $*.aux $*.idx && false ; } ; done
...
clean:
	for EXT in ${EXTS}; do \
	  find `pwd` -name \*\.$${EXT} -exec del -v \{\} \; ; done
